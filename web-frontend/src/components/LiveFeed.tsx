  ...
  return (
    <div className="live-feed cs2-card p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className={`w-3 h-3 rounded-full ${isConnected ? 'bg-green-400' : 'bg-gray-400'} animate-pulse`}></div>
      </div>

      {achievements.length > 0 ? (
        <div className="space-y-3 max-h-80 overflow-y-auto">
          {achievements.map((achievement, index) => (
            <div key={index} className="flex items-center gap-3 p-3 bg-black/20 rounded-lg border-l-4 border-l-cs2-orange">
              {achievement.type === 'headshot' ? (
                <Target className="w-5 h-5 text-cs2-orange flex-shrink-0" />
              ) : (
                <Zap className="w-5 h-5 text-cs2-blue flex-shrink-0" />
              )}
